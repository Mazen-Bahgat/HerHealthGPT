...
\hline
\end{tabular}
% TODO: Clarification required — this caption states "the scored evaluation set is
% English", but the paper now reports scored French and Arabic evaluation results
% (Table~\ref{tab:multilingual}, Section~\ref{sec:multilingual-results}). This
% appears to be a stale caption from an English-only draft. Please reconcile:
% either update the caption to reflect the multilingual evaluation, or clarify the
% intended scope. Left unedited pending your decision.
...
\begin{enumerate}
    \item preserving semantic intent while varying linguistic realization across registers and languages;
